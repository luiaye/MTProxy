...
PORT="${1:-33380}"
# 第二个参数：TLS 域名（Fake-TLS），默认 www.cloudflare.com
# 传空字符串禁用 Fake-TLS：bash install_mtproxy.sh 33380 ""
TLS_DOMAIN="${2:-www.cloudflare.com}"
...
echo "[1/9] 安装依赖..."
apt-get update -y
apt-get install -y git build-essential libssl-dev zlib1g-dev curl ca-certificates openssl xxd
...
ExecStart=$WORKDIR/objs/bin/mtproto-proxy -u nobody -p 8888 -H $PORT -S $SECRET --aes-pwd $WORKDIR/proxy-secret $WORKDIR/proxy-multi.conf $TLS_ARG
Restart=always
RestartSec=3
LimitNOFILE=65535
...
if [[ -n "$TLS_DOMAIN" ]]; then
  ...
  TG_SECRET="ee${SECRET}${DOMAIN_HEX}"
else
  TG_SECRET="dd${SECRET}"
fi

LINK="tg://proxy?server=${IP}&port=${PORT}&secret=${TG_SECRET}"

echo
echo "====== 安装完成 ======"
echo "PORT       : $PORT"
echo "SECRET     : $SECRET"
echo "TLS_DOMAIN : ${TLS_DOMAIN:-<disabled>}"
echo "链接       : ${LINK}"
